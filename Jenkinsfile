  ...
  stages {
    ...
    stage('Deploy') {
      steps {
        ...
        sh 'aws elasticbeanstalk create-environment --application-name alexacicd --environment-name Alexacicd-env --version-label "alexacicd-jenkins$BUILD_DISPLAY_NAME" --solution-stack-name "64bit Amazon Linux 2018.03 v2.16.6 running GlassFish 5.0 Java 8 (Preconfigured - Docker)" '
      }
    }
  }
}
